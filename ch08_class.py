...
# Object(객체)
#  + Class로 생성 된 구체화 된 객체 -> 인스턴트
#  + *****파이썬의 모든것은 객체(인스턴스)
#  ■ 기본자료형 (프리미티브 타입)  : 변수에 실제 값이 저장
#  ■ 객체자료형 (레퍼런스 타입)    : 변수에 실제값이 위치한 주소 저장
#  → C:       기본
#  → Java:    기본  +  객체
#  → Python:  객체
#  ※ NUL: 참조할 대상이 없는 경우, 값이 없다(X)
a = 4
b = a + 1
...
# ** 객체 사용 3단계 **
# Class(도면) -> 객체 생성 -> 인스턴스(결과물) -> 인스터스 사용
# 1. Class(도면) 작성
#  + 파스칼 표기법을 사용
...
#  + Class가 동일 하더라도 생성 된 인스턴스는 각각 다름
test = ChosunTest()
test1 = ChosunTest()
test2 = ChosunTest()
...
mem = Member()
mem.member_join("ljs1004", "이준서", "20")